...
def test_SHAPER_times(N_trials, N_batch, N_particles, R, beta, run_name):
    ...
    times = np.array(times)
    print(f"Average time for {N_trials} trials with {N_batch} batches and {N_particles} particles: {np.mean(times)}")
    np.save(f"/home/rikab/Documents/Research/SPECTER/Studies/Timing/Data/SHAPER_{N_trials}_{N_batch}_{N_particles}_{run_name}_times.npy", times)
...
def test_POT_times(N_trials, N_batch, N_particles, R, beta, run_name):
        ...
        times = np.array(times)
        print(f"Average time for {N_trials} trials with {N_batch} batches and {N_particles} particles: {np.mean(times)}")
        np.save(f"/home/rikab/Documents/Research/SPECTER/Studies/Timing/Data/POT_{N_trials}_{N_batch}_{N_particles}_{run_name}_times.npy", times)



batch_sizes = [1, 10, 100, 1000]
particle_sizes = np.logspace(0, 3, 10).astype(int)
